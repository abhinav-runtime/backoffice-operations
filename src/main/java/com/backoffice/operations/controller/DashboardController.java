...
public class DashboardController {
	...
	@GetMapping("/urls")
	public GenericResponseDTO<List<String>> getUrls(@RequestHeader(HttpHeaders.AUTHORIZATION)  String token) {
		List<String> urlList = Arrays.asList("http://182.18.138.199/Alizz/1.png", "http://182.18.138.199/Alizz/3.png",
	...
	public ResponseEntity<byte[]> getImage(@RequestHeader(HttpHeaders.AUTHORIZATION)  String token) throws IOException {
		List<String> urls = Arrays.asList("http://182.18.138.199/Alizz/1.png", "http://182.18.138.199/Alizz/3.png",
				"http://182.18.138.199/Alizz/2.png");
		...
		try {
			for (String url : urls) {
				byte[] imageBytes = webClientBuilder.build().get().uri(url).retrieve().bodyToMono(byte[].class).block();
	...
	public ResponseEntity<GenericResponseDTO<Object>> getAccountTransactions(@RequestParam String accountNumber,
	...
	public ResponseEntity<GenericResponseDTO<Object>> getCreditCardTransactions(
			@RequestParam(required = false) String fromDate, @RequestParam(required = false) String toDate,
			@RequestParam String uniqueKey, @RequestHeader(HttpHeaders.AUTHORIZATION)  String token) {
	...
	public ResponseEntity<GenericResponseDTO<Object>> getBlockedAmounts(@RequestParam String uniqueKey,
			@RequestParam String accountNumber, @RequestHeader(HttpHeaders.AUTHORIZATION)  String token) {
