...
public class CardPinVerifyServiceImpl implements CardPinVerifyService {
	...
	public ValidationResultDTO verifyCardPin(CardPinVerifyDTO cardPinVerifyDTO, String token) {
		int attempts = attemptsMap.getOrDefault(cardPinVerifyDTO.getUniqueKeyCivilID(), 0);
		...
		if (user.isPresent()) {
			...
			{
				//User is on cooldown
				 if (isUserOnCooldown(cardPinVerifyDTO.getUniqueKeyCivilID())) {
				...
				if (!storedPin.getStoredCardPin().equals(cardPinVerifyDTO.getCardPin())) {
					attempts++;
		            attemptsMap.put(cardPinVerifyDTO.getUniqueKeyCivilID(), attempts);
					...
					if (attempts >= cardPinMaxAttempts){
						...
						cooldownMap.put(cardPinVerifyDTO.getUniqueKeyCivilID(), LocalDateTime.now());
					...
					attemptsMap.remove(cardPinVerifyDTO.getUniqueKeyCivilID());
				}
