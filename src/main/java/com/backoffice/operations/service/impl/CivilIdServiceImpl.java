...
//		CivilIdParameter civilIdParameter = civilIdParameterRepository.findById(id).orElse(null);
	...
	public ResponseEntity<AccessTokenResponse> getToken() {
		...
		ResponseEntity<AccessTokenResponse> response = jwtAuthRestTemplate.exchange(tokenApiUrl, HttpMethod.POST,
				requestEntity, AccessTokenResponse.class);
...
//			responseDTO.setData(data);
	...
	public GenericResponseDTO<Object> sendOtp(Optional<CivilIdEntity> civilIdEntity,
			GenericResponseDTO<Object> responseDTO) {
		OtpEntity otpEntity = otpRepository.findByUniqueKeyCivilId(civilIdEntity.get().getId());
		try {
...
//					String newOtp = CommonUtils.generateRandomOtp();
	...
	private GenericResponseDTO<Object> getSMSResponseObject(String newOtp, GenericResponseDTO<Object> responseDTO,
			Optional<CivilIdEntity> civilIdEntity) {
		try {
			// GET Access token from M2P and save it into the DB.
			ResponseEntity<AccessTokenResponse> response = commonUtils.getToken();
			logger.info("response: {}", response.getBody());
...
//			smsRequest.setRecipients(request.getRecipientNumbers());
				...
				ResponseEntity<Object> smsResponseEntity = jwtAuthRestTemplate.exchange(smsNotify, HttpMethod.POST,
	...
	public Object blockUnblockCard(BlockUnblockActionDTO blockUnblockActionDTO) {
		...
		ResponseEntity<Object> response = basicAuthRestTemplate.postForEntity(blockUnblockURL, requestEntity,
				Object.class);
